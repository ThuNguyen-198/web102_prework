...
function addGamesToPage(games) {
  ...
  games.forEach((game) => {
    ...
    let gameToDisplay = document.createElement("div");
    // add the class game-card to the list
    gameToDisplay.classList.add("game-card");
    // set the inner HTML using a template literal to display some info
    // about each game
    gameToDisplay.innerHTML += `
        <img class="game-img" src=${game.img} />
        <p>${game.name}</p>
        <p>${game.description}</p>`;
    document.getElementById("games-container").append(gameToDisplay);
  });
...
const displayStr = `A total of $${GAMES_JSON.reduce((acc, game) => {
  return acc + game.pledged;
}, 0).toLocaleString("en-US")} has been raised for ${
  GAMES_JSON.length
} games. Currently, ${unfundedGames} ${
  unfundedGames > 1 ? "games" : game
} remain unfunded. We need your help to fund these amazing games!`;
// create a new DOM element containing the template string and append it to the description container
let displayStrContainer = document.createElement("p");
displayStrContainer.innerHTML = displayStr;
descriptionContainer.append(displayStrContainer);
